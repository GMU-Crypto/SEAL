...
int main(int argc, char *argv[]) {
    ...
    std::string view_comm = gen_random(2400);
    ...
    cout << "Receiving tape_commit.." << endl;
    uint32_t msgLength;
    recv(sock,&msgLength,sizeof(uint32_t),0); // Receive the message length
    std::cout << "tape_commit length: " <<msgLength <<endl;
    ...
    printf("tape_commit received\n");
    std::cout << "Actual tape_commit length: " <<temp.size() <<endl;
    ...
    //send ack
    msgLength = 2400;
    std::cout << "() Sending view_comm of length " << msgLength << endl;
    send(sock,&msgLength ,sizeof(uint32_t) ,0); // Send the message length
    send(sock,view_comm.c_str() ,msgLength ,0); // Send the message data 
    std::cout << "() view_comm sent." << endl;
